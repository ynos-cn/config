...
<template>
  ...
  <div class="modify" v-if="visible">
    <BaseCard :title="`角色管理${formData?.name ? `- ${formData?.name}` : ''}`">
      <template #addonAfter>
...
import { apiDelete, apiFind } from '@/api/system/role-service'
...
function doQuery() {
  let body: BaseParams<RoleStruct> = {
    body: {
      ...searchData.value
    },
